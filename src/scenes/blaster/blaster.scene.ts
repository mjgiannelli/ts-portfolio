...
export default class BlasterScene extends THREE.Scene {
  ...
  async initialize() {
    const targetMtl = await this.mtlLoader.loadAsync(
      'assets/models/three-d/targetA.mtl',
    );
    targetMtl.preload();

    this.bulletMtl = await this.mtlLoader.loadAsync(
      'assets/models/three-d/foamBulletB.mtl',
    );
  ...
  private async createTarget(mtl: MTLLoader.MaterialCreator) {
    ...
    const modelRoot = await this.objLoader.loadAsync(
      'assets/models/three-d/targetA.obj',
    );
  ...
  private async createBlaster() {
    const mtl = await this.mtlLoader.loadAsync(
      'assets/models/three-d/blasterG.mtl',
    );
    mtl.preload();
    const modelRoot = await this.objLoader.loadAsync(
      'assets/models/three-d/blasterG.obj',
    );
    return modelRoot;
  }
  ...
  private async createBullet() {
    ...
    const bulletModel = await this.objLoader.loadAsync(
      'assets/models/three-d/foamBulletB.obj',
    );
    ...
    for (let i = 0; i < this.bullets.length; i++) {
      ...
      if (b.getShouldRemove()) {
        ...
        for (let j = 0; j < this.targets.length; j++) {
          ...
          if (target.position.distanceToSquared(b.group.position) < 0.1275) {
            this.remove(b.group);
            this.bullets.splice(i, 1);
            i--;
